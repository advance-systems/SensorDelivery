...
import { X, Check, Plus, Minus, Info } from 'lucide-react-native';
import { THEME } from '../constants/theme';
import { Produto, TamanhoPizza, SaborPizza, BordaPizza } from '../types';
...
export const PizzaBuilderModal: React.FC<PizzaBuilderModalProps> = ({
  ...
  const [tamanhos, setTamanhos] = useState<TamanhoPizza[]>([]);
  const [sabores, setSabores] = useState<SaborPizza[]>([]);
  const [bordas, setBordas] = useState<BordaPizza[]>([]);
  const [carregando, setCarregando] = useState(true);

  const [tamanhoSelecionado, setTamanhoSelecionado] = useState<TamanhoPizza | null>(null);
  const [saboresSelecionados, setSaboresSelecionados] = useState<SaborPizza[]>([]);
  const [bordaSelecionada, setBordaSelecionada] = useState<BordaPizza | null>(null);
  ...
  const carregarOpcoes = async () => {
    ...
    try {
      ...
      if (saboresTamanhos.tamanhos?.length > 0) {
        // Seleciona tamanho médio ou o primeiro por padrão
  ...
  const handleToggleSabor = (sabor: SaborPizza) => {
    ...
    if (jaSelecionado) {
      ...
      if (saboresSelecionados.length < tamanhoSelecionado.maxSabores) {
        ...
        // Se excedeu o máximo, substitui o último
        setSaboresSelecionados((prev) => [...prev.slice(1), sabor]);
      }
    }
  };

  // Cálculo de Preço: Preço Base do Tamanho + Maior adicional de sabor (Regra padrão de pizzaria) + Borda
  const precoBase = tamanhoSelecionado ? tamanhoSelecionado.precoBase : (produto?.preco || 0);
  const maiorAdicionalSabor = saboresSelecionados.reduce(
    (max, s) => Math.max(max, s.precoAdicional || 0),
    0
  );
  ...
  const handleConfirmar = () => {
    ...
    adicionarItem({
      ...
      borda: bordaSelecionada || undefined,
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose}>
      <SafeAreaView style={styles.safeArea}>
                    ...
                    >
                      ...
                      <View style={styles.optionRight}>
                        <Text style={[styles.optionPrice, isSelected && styles.optionTextActive]}>
                          R$ {tam.precoBase.toFixed(2).replace('.', ',')}
                        </Text>
                        ...
                        <View style={styles.optionInfo}>
                          ...
                          {sabor.precoAdicional ? (
                            <Text style={styles.adicionalText}>
                              + R$ {sabor.precoAdicional.toFixed(2).replace('.', ',')}
                            </Text>
                          ) : null}
                        </View>
                    ...
                    return (
                      ...
                      >
                        ...
                        <View style={styles.optionRight}>
                          <Text style={[styles.optionPrice, isSelected && styles.optionTextActive]}>
                            {borda.preco > 0
                              ? `+ R$ ${borda.preco.toFixed(2).replace('.', ',')}`
                              : 'Grátis'}
                          </Text>
                          <View style={[styles.radioCircle, isSelected && styles.radioCircleActive]}>
                            {isSelected && <View style={styles.radioInner} />}
        ...
        {/* Barra Inferior com Quantidade e Adicionar ao Carrinho */}
